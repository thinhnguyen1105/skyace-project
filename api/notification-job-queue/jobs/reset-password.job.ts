...
const processingResetPasswordEmail = (jobInfo, done) => {
  const transporter = initNodeMailer();

  ejs.renderFile(path.join(__dirname + `../../../../../../static/email-template/reset-password.ejs`), {
    userName: jobInfo.userName,
    resetPasswordUrl: jobInfo.resetPasswordUrl,
  }, (err, data) => {
    if (err) {
