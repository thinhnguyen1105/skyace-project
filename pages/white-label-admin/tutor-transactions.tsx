...
class TransactionsPage extends React.Component<any, any> {
  ...
  saveRemarks = async () => {
    await getTransactionService().update({
      ...
      tutorRemark: this.state.remarks
    });
    this.props.editData({
      _id: this.state.selected ? (this.state.selected as any)._id : null,
      tutorRemark: this.state.remarks
  ...
  render() {
        ...
        <div className="action-buttons">
          ...
          <Tooltip title={'Download Invoice'}>
            <Button
              ...
              icon="download"
              className="button"
              onClick={() => {message.success('Under development')}}
            />
          </Tooltip>
        </div>
      );
    ...
    const columns = [
      ...
      {
        title: 'Commission due to Portal',
        ...
        render: (_value, record, _index) => {
          ...
          if (record.paymentDetail && record.paymentDetail.transactions) {
            ...
            return `${record.paymentDetail.transactions[0].amount.currency} ${total*0.25}`;
          } else {
            return 0;
          }
        },
      },
      {
        ...
        render: (_value, record, _index) => {
          ...
          if (record.paymentDetail && record.paymentDetail.transactions) {
            ...
            return `${record.paymentDetail.transactions[0].amount.currency} ${total*0.75}`;
          } else {
            return 0;
          }
        },
