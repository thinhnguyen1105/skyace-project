...
const myTuitionPageModel: ModelConfig<IMyTuiionPageState> = createModel({
  state: {
    isBusy: false,
    viewType: 'current',
  ...
  effects: {
    ...
    async fetchDataRequestCancel(payload: IFetchData, _rootState: any): Promise<void> {
      try {
        this.starting();
        ...
        let result: IFindTuitionsResult;
        if (payload.isStudent) {
          result = await tuitionsService.findDataRequestCancelByStudentId(
            payload.userId,
            payload.pageNumber,
            payload.pageSize,
            payload.sortBy,
          ...
          result = await tuitionsService.findDataRequestCancelByTutorId(
            payload.userId,
            payload.pageNumber,
            payload.pageSize,
            payload.sortBy,
...
export default myTuitionPageModel;