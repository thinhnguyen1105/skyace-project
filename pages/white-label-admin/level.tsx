...
class LevelPage extends React.Component<any, any> {
  ...
  handleOk = async () => {
    try {
      const newLevel = await getMetadataService().createLevel({
        ...this.props.inputs,
        slug: this.slugify(this.props.inputs.name),
        isActive: true
      });
      this.props.createSuccess(newLevel);
  ...
  handleUpdateOk = async () => {
    try {
      await getMetadataService().updateLevel(this.props.updateInputs);
  ...
  switchTag = async (record: any) => {
    try {
      await getMetadataService().toggleLevelActive({
        ...record,
...
const mapDispatch = (rootReducer: any) => {
  return {
    ...rootReducer.levelPageModel,
    profileReducers: rootReducer.profileModel,
    dataLookup: rootReducer.dataLookupModel
  };
};

export default withRematch(initStore, mapState, mapDispatch)(LevelPage);
